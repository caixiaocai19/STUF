...
<template>
  <view class="release_label">
    ...
    <Navigator-bar>
      <view
        slot="right"
        style="color: #efbe86; font-size: 32rpx;"
        @click="complete"
        >完成</view
      >
...
import { HADCHANGELABELS} from "../../constant.js";
import {
  getLabels,
  getAddLabel,
  getDeleteLabel,
...
export default {
      ...
      uni.showLoading({
        ...
        mask: true,
         success:async ()=>{
          let detLabels = [],
            addLabels = [];
          detLabels = this.initLabels.filter((label) => {
            //原来有的现在没有了就是删除了
            return this.selectedLabels.every((item) => {
              return item.labelId !== label.labelId;
            });
          });
          addLabels = this.selectedLabels.filter((label) => {
            //原来没有的现在有了就是增加了了
            return this.initLabels.every((item) => {
              return item.labelId !== label.labelId;
            });
          });
          addLabels = addLabels.map((item) => {
            return { labelId: item.labelId };
          });
          detLabels = detLabels.map((item) => {
            return { labelId: item.labelId };
          });
          try {
            await getAddLabel(addLabels);
            if (detLabels.length !== 0) {
              await getDeleteLabel(detLabels);
            }
            uni.showToast({
              title: "修改成功",
              duration: 1000,
            ...
            setTimeout(() => {
              uni.switchTab({
                ...
                success: () => {
                  //修改了用户标签
                  this.$bus.$emit(HADCHANGELABELS, this.selectedLabels);
                },
              });
            }, 1000);
          } catch (error) {
  ...
  onLoad(labels) {
    this.selectedLabels = JSON.parse(labels.labels);
    this.initLabels = JSON.parse(labels.labels);
  },
};
</script>
<style scoped>
