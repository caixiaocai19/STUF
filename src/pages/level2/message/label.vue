  ...
  <view class="release_label">
    ...
    <view class="show_selected">
      <view class="selected_item" v-for="(item,index) in selectedLabels"
        >{{item.content}}<image src="../../../../static/icons/message/delete.png"
    ...
    <view class="label">
      ...
      <view class="label_list">
        <view @click="chooseLabel(item)" class="label_item" v-for="(item, index) in labels[currentIndex].labelItem">{{item.content}}</view>
      </view>
    </view>
    <!-- 选择标签的面版结束 -->
  </view>
...
export default {
  ...
  methods: {
    ...
    //选择标签
    chooseLabel(item){
       if(this.selectedLabels.some((label)=>{
